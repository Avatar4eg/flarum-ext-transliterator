...
class TransliterateUrl
{
    ...
    public function createProperSlug(DiscussionWillBeSaved $event)
    {
        if (array_key_exists('title', $event->data['attributes'])) {
            $event->discussion->setAttribute('slug', self::urlSlug($event->discussion->getAttribute('title'), ['transliterate' => true]));
        }
    }
     ...
     * @return string
     */
    protected static function urlSlug($str, array $options = []) {
